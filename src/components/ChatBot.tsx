...
BOOKING:
- Leeds bookings through Fresha — book directly here: https://www.fresha.com/book-now/angelic-hairstyles-g0rg44br/all-offer?share=true&pId=122566
...
2. If asked about prices, give exact figures from above
3. When clients want to book, proactively share the Fresha link (for Leeds) or Instagram DM (for Dubai)
4. If asked something outside your knowledge, say you'll pass the question to Angelina personally
5. Keep responses concise but thorough — 2-4 sentences typically
6. Use British English spelling (colour, specialises, etc.)
7. Never invent services or prices not listed above`;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      assistantMessage = assistantMessage
        ...
        .replace(/https?:\/\/(?!www\.instagram\.com|www\.fresha\.com|wa\.me|fresha\.com)[^\s)]+/gi, "")
        .replace(/\n{3,}/g, "\n\n")
        .trim();
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
                  ...
                  >
                    <p className="font-body text-[13px] leading-[1.6] whitespace-pre-wrap">{msg.content}</p>
                  </div>
                </div>
              ))}
              {isLoading && (
